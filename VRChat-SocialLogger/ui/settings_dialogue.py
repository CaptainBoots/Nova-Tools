...
def open_settings(parent, cfg: dict, save_cb, reset_cb, theme_cb,
    ...
    skipped entirely, keeping this the one shared settings dialogue per
    ...
    def _do_reset_confirm():
        if QMessageBox.question(dlg, "Reset", "Reset all settings to defaults?") == QMessageBox.StandardButton.Yes:
            reset_cb()
