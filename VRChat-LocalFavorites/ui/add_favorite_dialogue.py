...
thread + relay), so the dialogue never freezes and never risks the
...
def open_add_favorite_dialogue(parent, category: str, api, store, on_added=None):
    """on_added(), if given, is called after a successful save — the
    caller (favorites_tab.py) uses this to refresh its list instead of
    this dialogue knowing anything about that widget."""
    dlg = QDialog(parent)
